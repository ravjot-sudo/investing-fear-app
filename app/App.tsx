...
/* ─────────────────────────────────────────────────────────
   SHARED ATOMS
───────────────────────────────────────────────────────── */
function Spin({ size=16, color=T.gold, style={} }) {
  return <div style={{width:size,height:size,border:`1.5px solid ${color}20`,borderTop:`1.5px solid ${color}`,borderRadius:"50%",animation:"spin .75s linear infinite",flexShrink:0,...style}}/>;
}
...
function SectionHead({ title, sub, action }: { title: any; sub?: any; action?: any }) {
...
function PCard({ children, onClick, style={}, glow }: { children: React.ReactNode; onClick?: any; style?: any; glow?: any }) {
...
IMPORTANT: Return ONLY valid JSON — no markdown, no backticks, no text before or after.
{
  ...
  return <div>
    ...
    <div style={{position:"relative",maxWidth:580,marginBottom:20}}>
      ...
      <input value={q} onChange={e=>setQ(e.target.value)} onFocus={e=>{e.target.style.borderColor=T.goldFoc;setSuggest(true);}} onBlur={e=>{e.target.style.borderColor=T.bdr;setTimeout(()=>setSuggest(false),200);}}
